<section>
    ...
    <div class="max-w-xl md:max-w-4xl mx-auto p-6 bg-white shadow-md  rounded-lg">
        ...
        <form class="space-y-4 w-full md:items-center md:justify-between md:flex md:flex-wrap "
            ...
            <!-- Bill Date -->
            <div wire:ignore class="md:w-[45%]">
                <x-form-label label_for="shop_name" star="true" input_label="Shop Name" />
                <select wire:model.live="shop_id" id="shop_name"
            ...
            <div class="md:w-[45%]">
                ...
                <x-form-input type="text" wire:model.live="date" id="date" placeholder="yy-mm-dd" readonly />
                        ...
                        <tbody class="w-full">
                            @forelse ($bills as $bill)
                                ...
                                <tr class="border-b odd:bg-white  even:bg-gray-100   border-gray-200  hover:bg-gray-200">
                                    <td class="px-6 text-center py-4">{{ $bill->id ? $bill->id : '-' }}</td>
                                    <td class="px-6 text-center py-4">{{ $bill->shop_id == 0 ? 'Other' : $bill->shops->shop_name }}
                                    </td>
                                    <td class="px-6 text-center py-4">
                                        <span
                                            class="{{ $bill->type == 1 ? 'text-green-500' : 'text-red-500' }}">{{ $bill->type == 1 ? 'Income' : 'Expense' }}</span>
                                    </td>
                                    <td class="px-6 text-center py-4">{{ $bill->amount ? $bill->amount : '-' }}</td>
                                    <td class="px-6 text-center py-4">{{ $bill->note ? $bill->note : '-' }}</td>
                                    <td class="px-6 text-center py-4">{{ $bill->date ? $bill->date : '-' }}</td>



                                    <td class="text-center px-4 py-4">
                                        <button type="button" wire:click="editUser({{ $bill->id }})"
                                            class="hover:bg-green-400 transition-all origin-bottom-left bg-green-500 px-3 py-1 text-slate-50 rounded">Edit
                                        </button>
                                        <button type="button" wire:click="openDeleteModel({{ $bill->id }})"
